 ...
 * 如需要支持Header、Footer、EmptyView、LoadMore等扩展功能，请使用{@link LiteAdapter}
 *
 * @param <T>
 */
public class SimpleAdapter<T> extends AbstractAdapter<T> {
    ...
    protected void bindFromViewInjector(@NonNull ViewHolder holder, int position) {
        final T item = mDataSet.get(position - getHeaderCount());
        ...
        injector.bindData(holder, item, position - getHeaderCount());
    }
    ...
    protected int getViewTypeFromInjectors(int position) {
        ...
        if (mViewInjectors.size() > 1) {
            ...
            int adjustPosition = position - getHeaderCount();
        ...
        if (mOnItemClickListener != null) {
            viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    int position = viewHolder.getLayoutPosition() - getHeaderCount();
    ...
    public SimpleAdapter<T> attachTo(RecyclerView recyclerView) {
    ...
    public static class Builder<D> {
        ...
        public SimpleAdapter.Builder<D> itemClickListener(@NonNull OnItemClickListener listener) {
            this.onItemClickListener = Precondition.checkNotNull(listener);
            return this;
        }

        public SimpleAdapter.Builder<D> itemLongClickListener(@NonNull OnItemLongClickListener listener) {
            this.onItemLongClickListener = Precondition.checkNotNull(listener);
            return this;
        }

        public SimpleAdapter.Builder<D> injectorFinder(@NonNull InjectorFinder<D> finder) {
        ...
        public SimpleAdapter.Builder<D> register(@NonNull ViewInjector<D> injector) {
        ...
        public SimpleAdapter<D> create() {
            return new SimpleAdapter<>(
                    injectors, injectorFinder, onItemClickListener, onItemLongClickListener
            );
        }
    }
}
